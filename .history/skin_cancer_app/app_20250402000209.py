...
def recognize_speech():
    """Recognizes speech and returns the detected command."""
    ...
    try:
        ...
        st.sidebar.write(f"You said: {command}")
        return command
    except sr.UnknownValueError:
        st.sidebar.write("Could not understand the command.")
    except sr.RequestError:
        st.sidebar.write("Error with the speech recognition service.")
    return None

def navigate_with_voice():
    """Maps voice commands to navigation options."""
    command = recognize_speech()
    if command:
        if "home" in command:
            return "Home"
        elif "skin check" in command or "detection" in command:
            return "Skin Cancer Detection"
        elif "patient info" in command or "patient portal" in command:
...
def navigation():
    with st.sidebar:
        ...
        st.sidebar.markdown("### Voice Navigation")
        if st.button("🎤 Use Voice Command"):
            voice_command = navigate_with_voice()
            if voice_command:

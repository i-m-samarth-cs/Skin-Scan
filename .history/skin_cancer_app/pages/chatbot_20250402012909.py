...
def show(translation_context=None):
    """Display the enhanced multilingual chatbot interface"""
    # Get translation function from context or use identity function
    t = translation_context.get("t", lambda x: x) if translation_context else lambda x: x
    translate_text = translation_context.get("translate_text", lambda x, y: x) if translation_context else lambda x, y: x
    
    # Page header with animation
    st.markdown(f"<h1 style='text-align: center; color: #2E86C1;'>{t('SkinScan Assistant')}</h1>", unsafe_allow_html=True)
    ...
    if 'chat_history' not in st.session_state:
        st.session_state.chat_history = [
            {"role": "assistant", "content": t("Hello! I'm the SkinScan Assistant. I can answer your questions about skin cancer, help you navigate the app, or provide general skin health information. How can I help you today?")}
        ]
    ...
    with st.sidebar:
        ...
        if st.session_state.use_gpt:
            api_key = st.text_input(
                ...
                type="password", 
                help=t("Enter your OpenAI API key to use GPT for more intelligent responses")
            )
        ...
        st.session_state.use_gpt = st.toggle(
            t("Use AI-powered responses"), 
            value=st.session_state.use_gpt,
            help=t("Switch between built-in FAQ system and OpenAI GPT for more advanced responses")
        )
        
        # Frequently asked questions section
        st.markdown(f"## {t('Frequently Asked Questions')}")
        
        # Get FAQ data and translate questions
        faq_data = get_faq_data()
        faq_questions = [q["question"] for q in faq_data["faqs"]]
        translated_questions = [translate_text(q, st.session_state.language) for q in faq_questions]
        
        # Display FAQ buttons
        for i, question in enumerate(translated_questions):
            if st.button(question, key=f"faq_{i}"):
                # Add FAQ question to chat history (use original English for processing)
                st.session_state.chat_history.append({"role": "user", "content": question})
                
                # Get response
                if st.session_state.use_gpt and st.session_state.openai_api_key:
                    # Use GPT for response
                    response = chat_with_gpt(faq_questions[i])
                else:
                    # Use built-in FAQ system
                    response = get_chatbot_response(faq_questions[i])
                
                # Translate response if needed
                if st.session_state.language != 'en':
                    response = translate_text(response, st.session_state.language)
        ...
        # Clear chat button
        if st.button(t("Clear Chat History")):
            st.session_state.chat_history = [
                {"role": "assistant", "content": t("Hello! I'm the SkinScan Assistant. I can answer your questions about skin cancer, help you navigate the app, or provide general skin health information. How can I help you today?")}
            ]
            st.rerun()
    ...
    user_input = st.chat_input(t("Type your question here..."))
    ...
    if user_input:
        ...
        if st.session_state.language != 'en':
            try:
                ...
                processing_input = result['translatedText']
            except Exception:
        ...
        with st.chat_message("assistant"):
            ...
            try:
                if st.session_state.use_gpt and st.session_state.openai_api_key:
                    # Use GPT for response
                    response = chat_with_gpt(processing_input)
                else:
                    # Use built-in FAQ system
                    response = get_chatbot_response(processing_input)
                ...
                error_message = t(f"Sorry, I encountered an error: {str(e)}")

...
import os
import sys
from app import get_asset_path  # Import the function from app.py
...
def show():
    ...
    with col2:
        # Try a different approach using absolute paths to fix the image issue
        try:
            # Method 1: Using the function from app.py
            hero_image_path = get_asset_path("images", "skin_scan_hero.jpg")
            st.image(hero_image_path, caption="Early detection is crucial")
        except:
            # Method 2: Alternative direct approach if the import fails
            try:
                # Get the absolute path based on this file's location
                current_file = os.path.abspath(__file__)
                pages_dir = os.path.dirname(current_file)
                project_root = os.path.dirname(pages_dir)
                image_path = os.path.join(project_root, "assets", "images", "skin_scan_hero.jpg")
                st.image(image_path, caption="Early detection is crucial")
            except Exception as e:
                st.error(f"Failed to load hero image: {str(e)}")
                st.info("The application will continue without the image.")
